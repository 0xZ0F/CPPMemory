...
int main() {
    DWORD procID = GetProcID("Testing.exe");
    MODULEENTRY32 mod = GetModule(procID, "Testing.exe");
    std::cout << mod.dwSize << std::endl;
    mod = GetModule(procID, "fsdf.exe");
    std::cout << mod.dwSize << std::endl;
...
static void Example_WriteToOffset() {
    ...
    MODULEENTRY32 modEntry = GetModule(procID, "Testing.exe");
...
static void Example_WriteToPattern() {
    ...
    void *patternAddr = PatternScanModule(hProc, "Testing.exe", "Testing.exe", "\x8B\x01\x3B\xC2\x7D\x03\x32\xC0\xC3\x2B\xC2\x89\x01\xB0\x01\xC3\xCC\x48\x89\x5C\x24\x08\x57\x48\x83\xEC\x20\x48\x8B\xF9\xE8\x0E\x3C\xF8\xFF", "xxxx??xxxxx?x????xxxx?xxxx?xxxx????");
